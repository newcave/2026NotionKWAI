...
_COPILOT_SYS = """당신은 K-water AI연구센터 대시보드 코파일럿입니다.
...
def _parse_copilot_command(user_input: str, api_key: str) -> dict:
    """자연어 → 구조화 액션. gpt-4o-mini 고정(코파일럿 파싱은 저비용 모델로)."""
    try:
...
def _execute_copilot_action(action: dict) -> str:
    """액션을 session_state에 반영하고 피드백 메시지 반환."""
    fn   = action["fn"]
    ...
    if fn == "select_member":
        ...
        return f"'{name}' 멤버를 찾을 수 없습니다. 등록된 이름을 확인해주세요."
        ...
        if target == "전체":
            st.session_state.copilot_trigger = "batch"
            return "✦ 전체 일괄 분석을 예약했습니다. 탭3(일괄처리)에서 '🚀 일괄 실행'을 눌러주세요."
        ...
        if name and status:
            ...
            if matched:
                ...
                return f"✦ '{st.session_state.members[matched[0]]['이름']}' 상태를 '{status}'로 변경했습니다."
        ...
        return f"오류: {args.get('message','알 수 없는 오류')}"
