...
/* ── [추가] 코파일럿 채팅 UI ── */
.copilot-panel{ background:#060d1a; border:1px solid #00f5ff22; border-radius:8px;
  padding:12px; margin-top:8px; }
.copilot-label{ font-family:'Orbitron',monospace; font-size:0.72rem; color:#00f5ff;
  letter-spacing:2px; margin-bottom:8px; }
.chat-msg-user{ background:#0d1a30; border-left:2px solid #00f5ff44; border-radius:4px;
  padding:6px 10px; margin:4px 0; font-size:0.78rem; color:#8ab4cc; }
.chat-msg-bot{ background:#071020; border-left:2px solid #00ff8844; border-radius:4px;
  padding:6px 10px; margin:4px 0; font-size:0.78rem; color:#b0d4e8; }
...
if "discovered_pages"   not in st.session_state: st.session_state.discovered_pages   = []
# ── [추가] 코파일럿 세션 키 ───────────────────────────────
if "copilot_history"    not in st.session_state: st.session_state.copilot_history    = []
if "copilot_member_sel" not in st.session_state: st.session_state.copilot_member_sel = None   # 코파일럿이 선택한 멤버
if "copilot_model_ovr"  not in st.session_state: st.session_state.copilot_model_ovr  = None   # 코파일럿 모델 오버라이드
if "copilot_trigger"    not in st.session_state: st.session_state.copilot_trigger    = None   # "summarize" | "batch" | "search"
if "copilot_search_q"   not in st.session_state: st.session_state.copilot_search_q   = ""

# ══════════════════════════════════════════════════════════════
# 코파일럿 함수 정의 — 사이드바보다 먼저 정의해야 호출 가능
# ══════════════════════════════════════════════════════════════

_COPILOT_TOOLS_EARLY = [
    {"type":"function","function":{
        "name":"select_member",
        "description":"특정 연구원을 선택하거나 필터링 (탭2 이동)",
        "parameters":{"type":"object","properties":{
            "name":{"type":"string","description":"연구원 이름. '전체' 입력 시 해제"}
        },"required":["name"]}
    }},
    {"type":"function","function":{
        "name":"run_ai_review",
        "description":"선택된 연구원 또는 전체 멤버를 AI로 분석",
        "parameters":{"type":"object","properties":{
            "target":{"type":"string","description":"연구원 이름 또는 '전체'"},
            "style":{"type":"string","enum":["소장검토","3줄요약","성과평가","추진과제"],
                     "description":"분석 스타일"}
        },"required":["target"]}
    }},
    {"type":"function","function":{
        "name":"change_model",
        "description":"AI 분석 모델 변경",
        "parameters":{"type":"object","properties":{
            "model":{"type":"string","enum":["gpt-4o","gpt-4o-mini","o3","o4-mini"]}
        },"required":["model"]}
    }},
    {"type":"function","function":{
        "name":"search_notion",
        "description":"Notion 페이지 검색",
        "parameters":{"type":"object","properties":{
    ...
    {"type":"function","function":{
        "name":"show_status",
        "description":"현재 멤버 목록과 검토 상태를 보고하거나, 특정 멤버의 상태 변경",
        "parameters":{"type":"object","properties":{
            "name":   {"type":"string","description":"연구원 이름 (생략 시 전체 현황)"},
            "status": {"type":"string","enum":["미검토","검토중","검토완료","보류"],
                       "description":"변경할 상태 (생략 시 현황만 보고)"}
        }}
    }},
]

_COPILOT_SYS_EARLY = """당신은 K-water AI연구센터 대시보드 코파일럿입니다.
사용자 명령을 아래 함수로 변환하세요. 연구원 이름이 불명확하면 select_member로 확인합니다.
명령이 일반 질문이면 show_status를 호출해 현황을 보여주세요."""

def _parse_copilot_command(user_input: str, api_key: str) -> dict:
    try:
        client = OpenAI(api_key=api_key)
        resp = client.chat.completions.create(
            model="gpt-4o-mini",
            messages=[
                {"role":"system","content":_COPILOT_SYS_EARLY},
                {"role":"user",  "content":user_input}
            ],
            tools=_COPILOT_TOOLS_EARLY,
            tool_choice="auto",
            max_tokens=200
        )
        ...
        if msg.tool_calls:
            tc = msg.tool_calls[0]
            return {"fn": tc.function.name, "args": json.loads(tc.function.arguments)}
        return {"fn":"chat","args":{"reply": msg.content or "명령을 인식하지 못했습니다."}}
    except Exception as e:
        return {"fn":"error","args":{"message": str(e)}}

def _execute_copilot_action(action: dict) -> str:
    fn   = action["fn"]
    ...
    if fn == "select_member":
        name = args.get("name","")
        if name == "전체":
            st.session_state.copilot_member_sel = None
            return "전체 멤버 보기로 전환했습니다. 탭2에서 확인하세요."
        matched = [m["이름"] for m in members if name in m["이름"]]
        if matched:
            st.session_state.copilot_member_sel = matched[0]
            return f"✦ '{matched[0]}' 선택 완료 → 탭2(개인 AI 검토)로 이동하세요."
        return f"'{name}' 멤버를 찾을 수 없습니다."

    elif fn == "run_ai_review":
        target = args.get("target","")
        style  = args.get("style","소장검토")
        if target == "전체":
            st.session_state.copilot_trigger = "batch"
            return "✦ 전체 일괄 분석 예약 → 탭3에서 실행하세요."
        matched = [m for m in members if target in m["이름"]]
        if matched:
            m = matched[0]
            st.session_state.copilot_member_sel = m["이름"]
            st.session_state.copilot_trigger    = f"summarize:{m['이름']}:{style}"
            return f"✦ '{m['이름']}' AI 검토({style}) 예약 → 탭2로 이동하세요."
        return f"'{target}' 멤버를 찾을 수 없습니다."

    elif fn == "change_model":
        model = args.get("model","gpt-4o")
        st.session_state.copilot_model_ovr = model
        desc = {"o3":"고추론","gpt-4o":"균형","o4-mini":"추론경량","gpt-4o-mini":"빠름"}
        return f"✦ 모델 → {model} ({desc.get(model,'')})"

    elif fn == "search_notion":
        keyword = args.get("keyword","")
        st.session_state.copilot_search_q = keyword
        st.session_state.copilot_trigger  = "search"
        return f"✦ '{keyword}' 검색 예약 → 탭4로 이동하세요."

    elif fn == "show_status":
        name   = args.get("name","")
        status = args.get("status","")
        if name and status:
            matched = [i for i,m in enumerate(members) if name in m["이름"]]
            if matched:
                st.session_state.members[matched[0]]["상태"] = status
                return f"✦ '{st.session_state.members[matched[0]]['이름']}' 상태 → '{status}'"
            return f"'{name}' 멤버를 찾을 수 없습니다."
        if members:
            lines = [f"{STATUS_EMOJI.get(m['상태'],'')} {m['이름']} ({m['직급']}) — {m['상태']}" for m in members]
            return "현황:\n" + "\n".join(lines)
        return "등록된 멤버가 없습니다."

    elif fn == "chat":
        return args.get("reply","명령을 처리했습니다.")
    elif fn == "error":
        return f"오류: {args.get('message','')}"
    return "처리 완료."
...
with st.sidebar:
    ...
    # [수정] 코파일럿 오버라이드 적용: copilot_model_ovr 가 있으면 selectbox 기본값 변경
    _ovr_model = st.session_state.copilot_model_ovr
    _default_idx = 0
    if _ovr_model:
        for _i, _k in enumerate(_model_keys):
            if MODEL_MAP[_k] == _ovr_model:
                _default_idx = _i
                break
    selected_model = MODEL_MAP[st.selectbox("모델", _model_keys, index=_default_idx)]
    ...
    # ══════════════════════════════════════════════════════════
    # [추가] AI 코파일럿 패널 (사이드바 하단)
    # ══════════════════════════════════════════════════════════
    st.divider()
    st.markdown('<div class="copilot-label">AI COPILOT</div>', unsafe_allow_html=True)
    st.caption("자연어로 대시보드를 제어하세요")

    # 대화 히스토리 표시 (최근 6개)
    if st.session_state.copilot_history:
        hist_html = ""
        for msg in st.session_state.copilot_history[-6:]:
            cls = "chat-msg-user" if msg["role"] == "user" else "chat-msg-bot"
            prefix = "▶ " if msg["role"] == "user" else "✦ "
            hist_html += f'<div class="{cls}">{prefix}{msg["content"]}</div>'
        st.markdown(f'<div class="copilot-panel">{hist_html}</div>', unsafe_allow_html=True)

    # 명령 입력
    copilot_input = st.chat_input("예: 홍길동 연구원 AI 검토해줘")
    if copilot_input and openai_key:
        _parsed = _parse_copilot_command(copilot_input, openai_key)  # 함수는 아래 정의
        _response = _execute_copilot_action(_parsed)
        st.session_state.copilot_history.append({"role":"user",    "content": copilot_input})
        st.session_state.copilot_history.append({"role":"assistant","content": _response})
        st.rerun()
    elif copilot_input and not openai_key:
        st.warning("OpenAI Key를 먼저 설정해주세요.")

    # 빠른 명령 버튼
    member_names = [m["이름"] for m in st.session_state.members]
    if member_names:
        st.markdown('<div style="font-size:0.72rem;color:#334455;margin-top:6px;">빠른 명령</div>', unsafe_allow_html=True)
        qc1, qc2 = st.columns(2)
        if qc1.button("전체 일괄분석", key="qc_batch",  use_container_width=True):
            st.session_state.copilot_trigger    = "batch"
            st.session_state.copilot_member_sel = None
            st.session_state.copilot_history.append({"role":"user",     "content":"전체 멤버 일괄 AI 분석"})
            st.session_state.copilot_history.append({"role":"assistant", "content":"✦ 탭3(일괄처리)에서 실행을 눌러주세요."})
            st.rerun()
        if qc2.button("히스토리 초기화", key="qc_clr", use_container_width=True):
...
def save_to_notion(title: str, content: str) -> bool:
    ...
    # Notion rich_text 블록 한 개당 최대 2000자 제한 → 청크 분할
    CHUNK = 1900
    chunks = [content[i:i+CHUNK] for i in range(0, len(content), CHUNK)]
    children = [
        {"object":"block","type":"heading_2",
         "heading_2":{"rich_text":[{"text":{"content": title[:2000]}}]}}
    ]
    for chunk in chunks:
        children.append({
    ...
    # Notion API는 한 번에 최대 100개 블록만 허용 → 100개씩 나눠 append
    BLOCK_LIMIT = 100
    for i in range(0, len(children), BLOCK_LIMIT):
        notion.blocks.children.append(
            block_id=save_page_id,
            children=children[i:i+BLOCK_LIMIT]
        )
    return True
...
# [추가] 코파일럿 트리거 알림 배너
if st.session_state.copilot_trigger:
    trigger = st.session_state.copilot_trigger
    if trigger == "search":
        st.info(f"💬 코파일럿: Notion 검색 예약됨 — 탭4에서 '{st.session_state.copilot_search_q}' 자동 검색 준비 완료")
    elif trigger == "batch":
        st.info("💬 코파일럿: 전체 일괄 분석 예약됨 — 탭3로 이동 후 실행하세요.")
    elif trigger and trigger.startswith("summarize:"):
        parts = trigger.split(":")
        st.info(f"💬 코파일럿: '{parts[1]}' AI 검토 예약됨 — 탭2로 이동하세요.")
...
if not st.session_state.members:
    ...
    if st.session_state.discovered_pages:
        ...
        st.caption("페이지 제목이 멤버 이름으로 자동 설정됩니다.")
        ...
        if sel_pages:
            ...
            if st.button("✅ 선택한 페이지를 멤버로 등록하고 시작", use_container_width=True):
                ...
                st.success(f"✅ {len(sel_pages)}명 등록 완료!")
...
with tab2:
    ...
    # [수정] 코파일럿이 멤버를 지정했으면 해당 멤버를 기본값으로
    _default_name = st.session_state.copilot_member_sel
    _default_idx  = names.index(_default_name) if _default_name in names else 0

    sel_name = st.selectbox("멤버 선택", names, index=_default_idx, key="tab2_sel")
    ...
    # [수정] 코파일럿 트리거에서 스타일 자동 선택
    _trig = st.session_state.copilot_trigger or ""
    _auto_style_idx = 0
    if _trig.startswith(f"summarize:{sel_name}:"):
        _style_key = _trig.split(":")[-1]
        _mapped = STYLE_MAP.get(_style_key,"")
        if _mapped in STYLE_OPTIONS:
            _auto_style_idx = STYLE_OPTIONS.index(_mapped)

    style_sel = st.selectbox("검토 관점", STYLE_OPTIONS, index=_auto_style_idx, key="tab2_style")
    ...
    # [수정] 코파일럿 트리거 시 자동 실행 안내 + 실행 버튼
    _auto_run = _trig.startswith(f"summarize:{sel_name}:")
    if _auto_run:
        st.info(f"💬 코파일럿이 '{sel_name}' AI 검토를 예약했습니다. 아래 버튼을 눌러 실행하세요.")
    ...
    if st.button(f"✨ {sel_name} AI 검토 실행", use_container_width=True):
        ...
        # [추가] 코파일럿 트리거 초기화
        if st.session_state.copilot_trigger and st.session_state.copilot_trigger.startswith("summarize:"):
            st.session_state.copilot_trigger = None
...
with tab3:
    ...
    # [추가] 코파일럿 batch 트리거 안내
    if st.session_state.copilot_trigger == "batch":
        st.info("💬 코파일럿이 전체 일괄 분석을 예약했습니다. 아래 '🚀 일괄 실행' 버튼을 눌러주세요.")
...
# ════════════════════════════════════════════════════════════
# TAB 4: 멤버 재탐색 — [수정] 코파일럿 검색 트리거 반영
# ════════════════════════════════════════════════════════════
with tab4:
    st.markdown("### 🔄 Notion 페이지 재탐색 / 멤버 추가")
    st.caption("새 Integration 연결 페이지를 탐색하거나 멤버를 추가합니다.")

    # [수정] 코파일럿 검색 키워드 자동 적용
    _search_default = st.session_state.copilot_search_q if st.session_state.copilot_trigger == "search" else ""

    col_sq, col_sbtn = st.columns([3,1])
    sq = col_sq.text_input("검색어", placeholder="이름 또는 키워드", key="rescan_q",
                            value=_search_default)
    _auto_search = st.session_state.copilot_trigger == "search"
    if col_sbtn.button("🔍 탐색", use_container_width=True, key="rescan_btn") or _auto_search:
        with st.spinner("탐색 중..."):
            pages = discover_notion_pages(sq or _search_default)
        st.session_state.discovered_pages = pages
        # [추가] 코파일럿 트리거 초기화
        if _auto_search:
            st.session_state.copilot_trigger   = None
            st.session_state.copilot_search_q  = ""
...
with tab5:
    ...
    if col_restart.button("⚠️ 멤버 전체 초기화 (처음부터)", use_container_width=True):
        ...
        st.session_state.copilot_trigger   = None
        st.session_state.copilot_member_sel = None
        st.session_state.copilot_model_ovr  = None
