...
# ══════════════════════════════════════════════════════════════
# 🔐 API 키 해석 우선순위
#   1순위: 사이드바 수동 입력 (입력된 경우)
#   2순위: st.secrets (Streamlit Cloud 비밀관리 / .streamlit/secrets.toml)
#   → 사이드바는 "오버라이드 전용" — 비어있으면 secrets 자동 사용
# ══════════════════════════════════════════════════════════════
...
.ai-result {
    background:#071020; border:1px solid #00f5ff33; border-radius:8px;
...
with st.sidebar:
    ...
    st.caption("비어두면 서버 secrets 자동 사용 / 입력 시 오버라이드")

    # secrets에서 기본값 로드
    _secret_notion  = _get_secret("NOTION_TOKEN")
    _secret_openai  = _get_secret("OPENAI_API_KEY")
    _secret_page_id = _get_secret("NOTION_PAGE_ID")

    # 사이드바 입력 (오버라이드용) — placeholder로 secrets 상태 표시
    notion_input = st.text_input(
        "🔑 Notion Token",
        type="password",
        placeholder="secret 자동사용 중 (오버라이드 시 입력)" if _secret_notion else "secret_xxx 입력 필요",
        help="비워두면 서버의 NOTION_TOKEN secrets를 사용합니다"
    )
    openai_input = st.text_input(
        "🤖 OpenAI API Key",
        type="password",
        placeholder="secret 자동사용 중 (오버라이드 시 입력)" if _secret_openai else "sk-xxx 입력 필요",
        help="비워두면 서버의 OPENAI_API_KEY secrets를 사용합니다"
    )

    # 최종 키 결정: 사이드바 입력 > secrets
    notion_token = notion_input  if notion_input  else _secret_notion
    openai_key   = openai_input  if openai_input  else _secret_openai

    # 키 상태 표시
    def _key_badge(label: str, val: str, is_override: bool) -> str:
        if not val:
            return f'<span class="key-no">✗ {label} 미설정</span>'
        if is_override:
            return f'<span class="key-ovr">⚡ {label} 오버라이드</span>'
        return f'<span class="key-ok">✓ {label} secrets 연결</span>'

    st.markdown(
        _key_badge("Notion",  notion_token, bool(notion_input))  + "<br>" +
        _key_badge("OpenAI",  openai_key,   bool(openai_input)),
        unsafe_allow_html=True
    )
...
- **gpt-4o-mini**: 빠른 초안, 대량 처리
    ...
    # Notion 저장 페이지 ID (사이드바 입력 > secrets)
    page_input     = st.text_input(
        "📄 Notion 저장 페이지 ID",
        placeholder="secret 자동사용 중" if _secret_page_id else "30531c56-8f2f-...",
    )
    notion_page_id = page_input if page_input else _secret_page_id

    st.markdown(
        f'<div style="font-family:Orbitron,monospace;font-size:0.7rem;color:#334455;margin-top:16px;">'
...
def call_ai(system: str, user: str) -> str:
    if not oai:
        return "⚠️ OpenAI API Key가 설정되지 않았습니다. 사이드바 또는 Streamlit secrets를 확인해주세요."
    kwargs = dict(
...
with tab1:
    ...
    st.markdown(
        f'<table class="cp-table"><thead><tr>'
        f'<th>이름</th><th>직급</th><th>주업무</th><th>세부내용</th><th>검토상태</th><th>AI요약</th>'
    ...
    if "team_report" in st.session_state:
        ...
        if st.button("💾 Notion에 종합보고서 저장", use_container_width=True, key="save_team_report"):
            if save_to_notion(
                f"[AI종합보고서] 2026 팀 업무현황 {datetime.now().strftime('%Y.%m.%d')}",
                st.session_state["team_report"]
            ):
                st.success("✅ Notion 저장 완료!")
            else:
                st.warning("Notion 설정을 사이드바 또는 secrets에서 확인해주세요.")
...
with tab2:
    ...
    with col_l:
        ...
        new_status = col_s1.selectbox(
            "검토 상태", STATUS_OPTIONS,
    ...
    with col_r:
        ...
        if sel_name in st.session_state.ai_outputs:
            st.markdown(
                '<div style="text-align:center;margin-top:12px;color:#00f5ff;'
                'font-family:Orbitron,monospace;font-size:0.75rem;">✦ AI 요약 완료</div>',
                unsafe_allow_html=True
            )
    ...
    STYLE_SYS = {
        "소장 검토 보고서  (업무요약·강점·개선·코멘트)": "[업무핵심요약] [강점분석] [개선제안] [소장코멘트] 형식으로 작성하세요.",
    ...
    if sel_name in st.session_state.ai_outputs:
        st.markdown(
            f'<div class="ai-result">{st.session_state.ai_outputs[sel_name]}</div>',
            unsafe_allow_html=True
        )
